...
rolling_horizon.enabled: false → Legacy-Pfad (MaintenanceSimulator), Log-Dir: logs/vfa
rolling_horizon.enabled: true  → RH-Pfad (RollingHorizonRunner),     Log-Dir: logs/vfa_rh
...
def main() -> None:
    ...
    parser.add_argument("--log-dir",      type=str, default=None,
                        help="Basisordner für JSON- und Log-Ausgaben "
                             "(Standard: logs/vfa bzw. logs/vfa_rh)")
    ...
    if args.log_dir is None:
        args.log_dir = "logs/vfa_rh" if rh_enabled else "logs/vfa"
    ...
    completed_rh: dict[int, tuple[int, int]] = {}
    ...
    for seed in seeds_to_run:
        ...
        if not rh_enabled:
            ...
            sim.write_json(result, str(out_path),
                           label="VFA SIMULATION", run_id=seed, model_params=model_params)

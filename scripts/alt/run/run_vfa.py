...
Monte Carlo:
    python scripts/run/run_vfa.py --output logs/vfa/run_1.json --run-id 1 --seed 1
"""
...
from src.models.alt.vfa import VFAModel, STATE_FEATURE_NAMES
...
def _build_model_params(cfg: dict, policy: VFAModel, local_theta: str, global_theta: str) -> dict:
    ...
    params = {
        ...
        "local_theta_path":      str(local_theta),
        "global_theta_path":     str(global_theta),
        "cost_params": {
...
def main() -> None:
    ...
    model_params = _build_model_params(cfg, policy, args.local_theta, args.global_theta)

    if not rh_enabled:
        # ----------------------------------------------------------------
        # Legacy-Pfad: bestehender MaintenanceSimulator (unverändert)
        # ----------------------------------------------------------------
        print(f"\nStarte VFA-Simulation [Legacy] (max. {args.max_days} Tage)...\n")
        ...
        # ----------------------------------------------------------------
        print(
            f"\nStarte VFA-Simulation [Rolling Horizon] "
        ...
        sep = "=" * 62
        print(sep)
        print("  VFA [Rolling Horizon] – ZUSAMMENFASSUNG")
        print(sep)
        ...
        runner.write_json(
            result, str(out_path),
            label="VFA SIMULATION [Rolling Horizon]",
            run_id=args.run_id,
            model_params=model_params,
            rh_config=rh_cfg,

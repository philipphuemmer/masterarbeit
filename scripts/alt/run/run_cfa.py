"""
Startet die CFA-Simulation mit gelernter Wertfunktionsapproximation.

Voraussetzung: θ muss zuerst trainiert werden:
    python scripts/train_cfa.py

Ausführen:
    python scripts/run_cfa.py
    python scripts/run_cfa.py --max-days 10 --log-day 1 --verbose

Monte Carlo:
    python scripts/run_cfa.py --output logs/cfa/run_1.json --run-id 1 --seed 1
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CFA-Simulation (gelernte Wertfunktion)")
    ...
    parser.add_argument("--output",     type=str,   default="logs/cfa/run_1.json",
    ...
    parser.add_argument("--theta-path", type=str,   default="data/training/cfa/theta.json",
                        help="Pfad zur theta.json (Standard: data/training/cfa/theta.json)")
    ...
    sim.print_summary(result, label="CFA")
    ...
    sim.write_json(result, str(out_path), label="CFA SIMULATION", run_id=args.run_id,

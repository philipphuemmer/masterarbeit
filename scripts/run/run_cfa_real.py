"""
Startet die CFA-Real-Simulation (echte Cost Function Approximation).

Voraussetzung: θ muss zuerst trainiert werden:
    python scripts/train/train_cfa.py

Ausführen:
    python scripts/run/run_cfa_real.py
    python scripts/run/run_cfa_real.py --max-days 10 --log-day 1 --verbose

Monte Carlo:
    python scripts/run/run_cfa_real.py --output logs/cfa_real/run_1.json --run-id 1 --seed 1
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CFA-Real-Simulation (AddDisjunction)")
    ...
    parser.add_argument("--output",     type=str,   default="logs/cfa_real/run_1.json",
    ...
    sim.write_json(result, str(out_path), label="CFA-REAL SIMULATION", run_id=args.run_id,

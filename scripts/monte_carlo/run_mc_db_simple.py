...
def _run_seed(params: dict) -> dict:
    ...
    default_delta = params["default_delta"]
    theta_path    = params["theta_path"]

    if no_model:
        db_model = DBSimpleBalanceModel(default_delta=default_delta)
    else:
        db_model = DBSimpleBalanceModel.load(Path(model_path), default_delta=default_delta)
    ...
    policy = DBSimplePolicy(
        traffic_matrices=mats, config=run_cfg, all_coords=coords,
        stations_df=df_base, db_model=db_model,
        default_delta=default_delta, theta_path=theta_path,
    )
    ...
    if not rh_enabled:
        ...
        sim.write_json(result, str(out_path), label="DB-SIMPLE SIMULATION", run_id=seed, model_params=model_params)
        ...
        def _pre_day_hook(state, _policy=policy, _cfg=run_cfg) -> None:
            phi = _policy.extract_balance_features(state.remaining, state.days_since_maintenance, state.day)
            _dm = _cfg.get("db_simple", {}).get("delta_mode", "rf")
            if _dm == "rule":
        ...
        runner.pre_day_hook = _pre_day_hook
        ...
        runner.write_json(result, str(out_path), label="DB-SIMPLE SIMULATION [Rolling Horizon]",
                          run_id=seed, model_params=model_params, rh_config=rh_cfg,
                          replan_overrides=replan_ov, initial_overrides=initial_ov)
        runner.write_log(result, str(log_path), label="DB-SIMPLE SIMULATION [Rolling Horizon]")
...
def main() -> None:
    ...
    with multiprocessing.Pool(n_workers) as pool:
        for res in pool.imap_unordered(_run_seed, worker_params):
            ...
            days = res["days_to_complete"] or "?"
            print(f"  Lauf {seed} fertig ({len(completed)}/{args.runs} gesamt) | "
                  f"{res['total_cost_eur']:>12,.2f} € | Tage: {str(days):>4}")

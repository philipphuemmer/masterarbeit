...
def _run_seed(params: dict) -> dict:
    ...
    if not rh_enabled:
        ...
        sim.write_json(result, str(out_path), label="MYOPIC PLUS SIMULATION", run_id=seed, model_params=model_params)
        ...
        runner.write_json(result, str(out_path), label="MYOPIC PLUS SIMULATION [Rolling Horizon]",
                          run_id=seed, model_params=model_params, rh_config=rh_cfg,
                          replan_overrides=replan_ov, initial_overrides=initial_ov)
        runner.write_log(result, str(log_path), label="MYOPIC PLUS SIMULATION [Rolling Horizon]")

"""
Monte-Carlo-Simulation für die CFA-Real-Policy (AddDisjunction).

Führt N Läufe durch (Seed 1 … N), speichert jeden Lauf als
logs/cfa_real/json/run_<N>.json und logs/cfa_real/log/run_<N>.log.
Am Ende wird eine aggregierte Analyse als logs/cfa_real/log/cfa_real_overview.log gespeichert.

Voraussetzung: theta muss trainiert sein:
    python scripts/train/train_cfa.py

Ausführen:
    .venv/bin/python3 scripts/monte_carlo/run_mc_cfa_real.py --runs 30
    .venv/bin/python3 scripts/monte_carlo/run_mc_cfa_real.py --runs 10 --max-days 50 --verbose
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Monte-Carlo-Simulation (CFA-Real)")
    ...
    parser.add_argument("--theta-path", type=str, default="data/cfa_real/theta.json",
                        help="Pfad zur theta.json (Standard: data/cfa_real/theta.json)")
    parser.add_argument("--log-dir",    type=str, default="logs/cfa_real",
                        help="Basisordner für JSON- und Log-Ausgaben (Standard: logs/cfa_real)")
    ...
    for seed in seeds:
        ...
        sim.write_json(result, str(out_path), label="CFA-REAL SIMULATION", run_id=seed,
                       model_params=model_params)

        log_path = log_dir / f"run_{seed}.log"
        sim.write_log(result, str(log_path), label="CFA-REAL SIMULATION")

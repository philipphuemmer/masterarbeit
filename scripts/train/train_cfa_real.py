"""
CFA-Real-Training: Lernt θ über Policy Iteration mit CFARealModel.

Identische Regression wie train_cfa.py, aber Runde 2+ verwendet CFARealModel
(AddDisjunction) statt CFAModel (Soft-Deadlines), damit θ gegen die eigene
Policy konvergiert.
...
Training:
    ...
    Runde 2: N CFAReal(θ₁)-Simulationen → θ₂
    Runde r: N CFAReal(θ_{r-1})-Simulationen → θ_r

Ausführen:
    python scripts/train/train_cfa_real.py
    python scripts/train/train_cfa_real.py --runs 20 --rounds 3 --max-days 200
"""
...
class CFARealTrainingSimulator(MaintenanceSimulator):
    """Zeichnet pro Tag das Feature Σ power×dsm für die Regression auf."""
...
def fit_theta(X: np.ndarray, y: np.ndarray) -> tuple[float, float, float]:
    """OLS: cost_to_go ≈ θ × feature + intercept. Gibt (theta, intercept, r2) zurück."""
    A = np.column_stack([X, np.ones(len(X))])
    coeffs, _, _, _ = np.linalg.lstsq(A, y, rcond=None)
    theta_val, intercept_val = float(coeffs[0]), float(coeffs[1])
...
def run_round(
    ...
    round_idx >= 2  → CFARealModel mit theta_prev
    """
    X_all: list[float] = []
    y_all: list[float] = []
    use_value_based = cfg["planning"].get("value_based_zone_selection", False)

    label = "Myopic" if round_idx == 1 else f"CFAReal(θ={theta_prev:.3e})"
    ...
    for run_i, seed in enumerate(seeds, 1):
        ...
        sim = CFARealTrainingSimulator(policy, selector, coords, df_base, mats, run_cfg)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CFA-Real-Training (Policy Iteration mit AddDisjunction)")
    ...
    parser.add_argument("--out",      type=str, default="data/cfa_real/theta.json",
                        help="Ausgabepfad für θ (Standard: data/cfa_real/theta.json)")
    ...
    if cfg.get("failure_simulation", {}).get("mode", "csv") != "stochastic":
        print("FEHLER: CFA-Real-Training erfordert failure_simulation.mode = stochastic.")
        sys.exit(1)
    ...
    print(f"\nIteratives CFA-Real-Training: Runde {start_round}–{args.rounds}, {args.runs} Läufe/Runde")

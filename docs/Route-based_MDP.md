...
- Zonen: $Z = \{1, \ldots, N_Z\}$, K-Means-Cluster der Stationen ($N_Z = 40$)

---

## Entscheidungspunkte

Die Simulation läuft auf einem **stündlichen Gitter** $\mathcal{H} = \{8, 9, \ldots, 16\}$ Uhr. Es gibt zwei Typen von Entscheidungspunkten:
...
> **Hinweis:** Der technische Zustand einer Säule (ok / gestört) wird nicht als persistente Zustandsgröße modelliert. Störungen werden als Ereignisse generiert und sofort als Wartungsauftrag in $Z_h$ eingestellt.

### 4. Offene Wartungsaufträge $Z_h$

$$Z_h = (W_h^{reg},\; W_h^{fault},\; W_h^{carry})$$
...
Für jede Stunde $h \in \mathcal{H}$ und Säule $i \in I$ (max. eine Störung pro Säule und Tag):

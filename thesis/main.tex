...
\end{figure}
To illustrate why both the initial plan and the replanning decision require more than routing efficiency, consider a simplified scenario with four stations — A, B, C, and D — and a central depot. Station B has not been serviced in 365 days and operates at 80 kW, placing it at maximum failure risk. Station A was serviced only 30 days ago and carries only 11 kW — still non-critical. Stations C and D have intermediate maintenance states with 100 and 200 days since last service respectively.\\
...
Section 3.1.1 defines maintenance optimization and its objectives.
Section 3.1.2 reviews maintenance strategies: preventive, reactive, and condition-based.
Section 3.1.3 discusses uncertainty, availability, and cost as central decision variables.
Section 3.1.4 provides the transition to mobile maintenance and service interventions (de Jonge \&
...
For this thesis, the key insight from Section 3.1 is that maintenance optimization provides the
decision objectives (cost, availability, uncertainty handling), while Sections 3.2 and 3.3 provide
...
Route-based MDPs are particularly important because they model the routing problem at the level of the route rather than only at the level of individual states or customer requests (Ulmer et al., 2019; Ulmer, 2020). The strongly represented Ulmer works in the research matrix use this perspective to systematically formulate dynamic routing problems as sequential decision problems (Ulmer et al., 2019; Ulmer, 2020). The advantage lies in the fact that the route itself is understood as a decision unit, and not only individual stops are considered in isolation (Ulmer et al., 2019).
...
The dynamic and stochastic nature of real-world service planning requires decision frameworks that can handle sequential choices under uncertainty without prohibitive computational costs. Exact Dynamic Programming (DP) provides a theoretically optimal foundation but quickly becomes intractable for realistic routing problems. Approximate Dynamic Programming (ADP) addresses this limitation by approximating future costs or values rather than computing them exactly, thereby enabling scalable, anticipative decision-making in operational settings. This chapter introduces the core concepts, methods, and relevance of ADP for dynamic routing. 
...
The Ulmer papers use this logic to evaluate routing decisions with expected future quality (Ulmer, 2017; Ulmer et al., 2019). For this work, this way of thinking is useful because maintenance interventions also start from a current state, and the quality of a decision only shows itself over the future. In mathematical terms, ADP approximates the Bellman equation:
\[
...
\subsubsection{Important ADP Approaches: Rollout, VFA, Offline-Online ADP, Post-decision States}
...
Offline-Online ADP combines an offline learned or precomputed value function with an online executed decision rule (Ulmer et al., 2019). This is particularly attractive because computational effort is shifted into the preparation phase while remaining responsive online (Ulmer, 2019; Ulmer, 2020). The Ulmer works from 2019 and 2020 are especially important here because they show exactly this combination as powerful for stochastic dynamic routing problems (Ulmer, 2020; Ulmer et al., 2019).
The offline-online policy enhances the anticipation of the VFA policy, yielding spatial and temporal anticipation of requests and routing developments (Ulmer et al., 2019). Our combination of VFA with rollout algorithms demonstrates the potential benefit of using offline and online methods in tandem as a hybrid ADP procedure, making possible higher-quality policies with reduced computational requirements for real-time decision making (Ulmer et al., 2019, p. 185).
Post-decision States extend this logic by considering not only the state before an action but the state directly after the action, before uncertainty enters (Anuar, Lee, \& Low, 2022; Ulmer, 2017). This is conceptually very helpful because it cleanly separates transition uncertainty from decision effect (Anuar et al., 2022). The paper on the Multi-Depot Problem with stochastic road capacity is a good example because there Post-Decision-State Rollout is used as dynamic decision logic (Anuar et al., 2022).
For this thesis, these four approaches are central because they show different ways to handle future uncertainty without solving a full exact DP (Ulmer, 2017; Ulmer et al., 2019). Particularly the combination of offline-VFA and online-rollout is very plausible for this problem setting because it is anticipative and simultaneously operationally fast (Ulmer, 2020; Ulmer et al., 2019).

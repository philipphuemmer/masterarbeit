...
For the advanced policies (Myopic+, CFA, DB), the Monte Carlo evaluation confirms rather than corrects the base policy's drop choice: their drop-score functions already incorporate station value, so even when the rollout disagrees (averaging 1.2--1.3 overrides per run out of $\approx$48 carryover decisions) the cost consequence is too small to accumulate into a measurable aggregate difference.
...
Under value-based zone selection, policy differences become statistically detectable but remain practically limited. Myopic differs from the three advanced policies by $d = 0.17$--$0.37$; Myopic+ differs from CFA and DB by $d = 0.16$--$0.20$. The residual gap has two interacting sources. First, different value signals produce different zone assignments even within value-based mode: Myopic's simple power--recency proxy and CFA's learned four-feature approximation $\tilde{C}$ direct teams to different geographic areas. Second, Myopic's power-agnostic routing ($1/\text{distance}$, $1/\text{detour}$) visits and protects stations in a different order than CFA's value-weighted scoring. Together they create a DSM cascade: CFA reduces days-since-maintenance on the highest-power stations faster, lowering their subsequent failure probability (Section~7.2). The combined effect is statistically detectable but economically modest. Notably, CFA and DB are statistically indistinguishable ($d = 0.05$, $p = 0.26$): the additional flexibility introduced by DB's distance-weighting parameter $\delta$ provides no measurable benefit over CFA's fixed formulation in the current experimental setting.\\
